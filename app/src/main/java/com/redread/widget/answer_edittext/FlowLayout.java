package com.redread.widget.answer_edittext;
...
public class FlowLayout extends ViewGroup {
    ...
    private int mUsefulWidth; // the space of a line we can use(line's width minus the sum of left and right padding
    private int mLineSpacing = 0; // the spacing between lines in flowlayout
    ...
    public FlowLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mLineSpacing = mTypedArray.getDimensionPixelSize(
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        int lineY = mPaddingTop;
        int lineWidth = r - l;
        mUsefulWidth = lineWidth - mPaddingLeft - mPaddingRight;
        int lineUsed = mPaddingLeft + mPaddingRight;
        int lineHeight = 0;
        for (int i = 0; i < this.getChildCount(); i++) {
            View child = this.getChildAt(i);
    ...
    private void sortToCompress(View[] childs, int[] spaces) {
        ...
        int[][] table = new int[childCount + 1][mUsefulWidth + 1];
        for (int i = 0; i < childCount +1; i++) {
            for (int j = 0; j < mUsefulWidth; j++) {
                table[i][j] = 0;
            }
        }
        ...
        for (int i = 1; i <= childCount; i++) {
            for (int j = spaces[i-1]; j <= mUsefulWidth; j++) {
    ...
    public void relayoutToAlign() {
        ...
        for (int i = 0; i < count; i++) {
            if (lineTotal + spaces[i] > mUsefulWidth) {
                int blankWidth = mUsefulWidth - lineTotal;
                int end = i - 1;
