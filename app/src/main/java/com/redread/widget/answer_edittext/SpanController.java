...
public class SpanController {
    ...
    public void makeData(ReplaceSpan.OnClick ac, TextView tv, String str){
        makeData(ac, tv, str,null);
    }
    //造对应的sentence
    public void makeData(ReplaceSpan.OnClick ac, TextView tv, String str,String ansStr){
        mAnswer=ansStr;
        ...
        try{
            ...
            mStr = str.replace("[","").replace("]","").replace("【","").replace("】","");
            //这里根据答案的长度来处理，选最长答案
            String[] split ;
            if(ansStr==null)
                split= mStr.split(" ");//NOTE 这里是为了处理单词的，对汉字没有影响
            else
                split= ansStr.split(",");
            int len =0;
